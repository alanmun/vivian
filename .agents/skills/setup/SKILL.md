...
## 4. Codex/OpenAI Authentication (No Script)

If HAS_ENV=true from step 2, read `.env` and check for `CODEX_API_KEY` or `OPENAI_API_KEY`. If present, confirm with user: keep or reconfigure?
...
**Delegate to each selected channel's own skill instructions.** Each channel skill handles its own code installation, authentication, registration, and JID resolution. This avoids duplicating channel-specific logic and ensures JIDs are always correct.

For each selected channel, read and execute the channel SKILL workflow directly:

- **WhatsApp:** `.agents/skills/add-whatsapp/SKILL.md`
- **Telegram:** `.agents/skills/add-telegram/SKILL.md`
- **Slack:** `.agents/skills/add-slack/SKILL.md`
- **Discord:** `.agents/skills/add-discord/SKILL.md`

Apply channel code with:

```bash
npx tsx scripts/apply-skill.ts .agents/skills/add-<channel>
```
...
- CHANNEL_AUTH shows `not_found` for any channel → re-run that channel's SKILL.md flow (e.g. `.agents/skills/add-telegram/SKILL.md`)
...
**Container agent fails ("Codex exited with code X"):** Ensure the container runtime is running — `open -a Docker` (macOS Docker), `container system start` (Apple Container), or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.
