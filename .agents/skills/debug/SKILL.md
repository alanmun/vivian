...
src/container-runner.ts               container/agent-runner/
    │                                      │
    │ spawns container                      │ runs Codex runtime
...
| **agent sessions** | `~/.codex/projects/` | Codex session history |
...
# CODEX_API_KEY=sk-ant-oat01-...  (subscription)
# CODEX_API_KEY=sk-ant-api03-...        (pay-per-use)
```
...
**Workaround:** The system extracts only authentication variables (`CODEX_API_KEY`, `CODEX_API_KEY`) from `.env` and mounts them for sourcing inside the container. Other env vars are not exposed.
...
echo '{}' | docker run -i \
  ...
  -c 'export $(cat /workspace/env-dir/env | xargs); echo "OAuth: ${#CODEX_API_KEY} chars, API: ${#CODEX_API_KEY} chars"'
```
...
```
/workspace/
├── env-dir/env           # Environment file (CODEX_API_KEY or CODEX_API_KEY)
├── group/                # Current group folder (cwd)
├── project/              # Project root (main channel only)
├── global/               # Global SOUL.md (non-main only)
...
grep -A3 "agent sessions" src/container-runner.ts
```
...
The agent-runner uses these Codex runtime options:
...
agent sessions are stored per-group in `data/sessions/{group}/.codex/` for security isolation. Each group has its own session directory, preventing cross-group access to conversation history.
...
[ -f .env ] && (grep -q "CODEX_API_KEY=sk-" .env || grep -q "CODEX_API_KEY=sk-" .env) && echo "OK" || echo "MISSING - add CODEX_API_KEY or CODEX_API_KEY to .env"
