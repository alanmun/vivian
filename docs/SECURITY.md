...
Each group has isolated agent sessions at `data/sessions/{group}/.codex/`:
...
**Mounted Credentials:**
- agent auth tokens (filtered from `.env`, read-only)
...
> **Note:** model provider credentials are mounted so that Codex can authenticate when the agent runs. However, this means the agent itself can discover these credentials via Bash or file operations. Ideally, Codex would authenticate without exposing credentials to the agent's execution environment, but I couldn't figure this out. **PRs welcome** if you have ideas for credential isolation.
